...
import 'package:SocialMediaClient/requests.dart';
...
void main() async {
  ...
  print(chalk.bgBlueBright(
      '1:Login, 2:Signup, 3:Post, 4:Add Post, 5:Chatroom, for exit press other keys'));
  chatListener = channel.stream.listen(
    (message) {
      if (currentPage == Pages.chatroom) {
        print(chalk.magenta('Message: $message'));
      }
    },
  );
...
void gotoChatroom() {
  print("You are in chat room for exit send -");
